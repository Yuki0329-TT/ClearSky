...
DOTween と uniBulletHell を活用しています。
...
### ゲーム設計面
スコアを伸ばすか、消費して強化を優先するかといった
リスクとリターンの駆け引きが生まれる設計を意識しました。
復活コスト・アイテム価格上昇・スコア倍率を組み合わせ、
プレイヤーが常に選択を迫られるゲームテンポを構築しています。

### オブジェクトプール
自作実装を試作しましたが、使用アセットとの競合による
管理の複雑化が課題となり、
安定性・保守性を優先し既存のプール構成を採用しています。

### 敵構成
初期の敵処理は1クラスに集約され責務過多となっていたため、
EnemyCore・Knockback・Animation・FormControllerへ分割しました。
現在は雑魚敵に分離後構成を適用済みで、
ボス処理については運用安定性を優先し従来構成を使用しています。
