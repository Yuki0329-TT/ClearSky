...
{
    [System.Serializable]
    public class OnFiredBulletEvent : UnityEvent<UbhBullet> { }
    ...
    // "Flag to use fired bullet callback."
    public bool m_useBulletFiredCallback = false;
    // "Set a callback method fired bullet."
    public OnFiredBulletEvent m_bulletFiredCallbackEvents = new OnFiredBulletEvent();

    protected bool m_shooting;
    public bool isShooting => m_shooting;
    private UbhShotCtrl m_shotCtrl;
    public virtual void StopShot() //とりあえず宣言だけ
    {
        FinishedShot();
        Debug.Log("弾変更のための弾停止");
    }
                               ...
                               bool sinWave = false, float sinWaveSpeed = 0f, float sinWaveRangeSize = 0f, bool sinWaveInverse = false)
    {
        ...
        if (m_useBulletFiredCallback)
        {
            m_bulletFiredCallbackEvents.Invoke(bullet);
        }
    }
}